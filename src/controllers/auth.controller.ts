...
import type { IUserRepository } from "../models/user.repository.ts";
...
export function createAuthController(repo: IUserRepository) {
  return (
        ...
        ({ body, jwt, set }) =>
          run(
            ...
            Effect.gen(function* () {
              ...
              const existingEither = yield* Effect.either(repo.findUserByEmail(body.email));
      ...
      .post(
        ...
        async ({ body, jwt, set }) => {
          ...
          const either = await Effect.runPromise(
            Effect.either(
              pipe(
                repo.findUserByEmail(body.email),
                Effect.flatMap((user) =>
                  Effect.gen(function* () {
