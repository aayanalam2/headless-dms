...
export class DrizzleAccessPolicyRepository implements IAccessPolicyRepository {
  ...
  private static fromRow(row: AccessPolicyRow): AccessPolicy {
    return AccessPolicy.reconstitute(
      AccessPolicyId.create(row.id).unwrap(),
      row.createdAt,
      {
        documentId: DocumentId.create(row.documentId).unwrap(),
        subjectId: row.subjectId
          ? Option.some(UserId.create(row.subjectId).unwrap())
          : Option.none(),
        subjectRole: row.subjectRole
          ? Option.some(row.subjectRole)
          : Option.none(),
        action: row.action,
        effect: row.effect,
      },
  ...
  findByDocument(documentId: DocumentId) {
    return fetchMultiple(
      () => this.db.select().from(accessPoliciesTable).where(eq(accessPoliciesTable.documentId, documentId)),
    ...
    return fetchMultiple(
      () =>
        this.db
          ...
          .where(and(eq(accessPoliciesTable.documentId, documentId), eq(accessPoliciesTable.subjectId, userId))),
      DrizzleAccessPolicyRepository.fromRow,
    );
  }
    ...
    return fetchMultiple(
      () =>
        this.db
          .select()
          .from(accessPoliciesTable)
          .where(and(eq(accessPoliciesTable.documentId, documentId), eq(accessPoliciesTable.subjectRole, role))),
      DrizzleAccessPolicyRepository.fromRow,
    );
  }
  ...
  delete(id: AccessPolicyId) {
    return Effect.flatMap(
      ...
      (rows) =>
        rows.length > 0
          ? Effect.void
          : Effect.fail(new AccessPolicyNotFoundError(id)),
    );
  }
